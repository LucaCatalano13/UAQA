...
class PixelTimeSeriesLabeled(PixelTimeSeries):
    ...
    def __getitem__(self, index):
        ...
        if (end_t + 1) < len(self.stations.files_temporal_aligned): 
            ...
            date = self.data[3][len(self.stations.files_temporal_aligned) - 1, row_ix, col_ix]
            label , loss_factor = self.stations.get_item_temporal_aligned(len(self.stations.files_temporal_aligned) - 1, row_ix, col_ix, date, latlon)
